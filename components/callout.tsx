...
export default function Callout({ type, children }: CalloutProps) {
  ...
  return (
    <div className={`text-${color}-800 border-l-4 border-${color}-800 bg-${color}-100 pl-2 py-4 mb-4 dark:bg-${color}-800 dark:text-${color}-50 dark:border-${color}-300 rounded`}>{children}</div>
  )
}
